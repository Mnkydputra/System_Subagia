...
class P_requirment  extends CI_Controller
{
	...
	function index()
	{
		$data = array(
			...
			'order'		=> $this->inventory_model->sc_control("i_order_rq")->result()
	...
	function p_rq($no_order)
	{
		$data = array(
			'akun' => $this->db->get_where('akun', array('email' => $this->session->userdata('email')))->row(),
			'url'  => $this->uri->segment(2),
			'stock' => $this->inventory_model->sc_control('i_data_requirment')->result(),
			'no_order'	=> $no_order
		);
		$this->load->view('template/header', $data);
	...
	public function shipDO()
	{
		...
		$updateAWB = $this->inventory_model->update(
			[
				'awb'		=> $awb
			],
			"i_order_rq",
	...
	function CetakDO($awb)
	{
		...
		$data2 = array(
			...
			'delivery'   => $this->inventory_model->cari(array("no_order" => $awb), "i_order_rq")->row(),
			'delivery_order'   => $this->inventory_model->cari(array("po_number" => $awb), "i_delivery_order")->result()
		);
